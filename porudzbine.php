...
<script>
    ...
    function render() {
        ...
        const niz = porudzbina.filter(element => {
            ...
            $(`#row-${red}`).append(
                `
                        <div class='col-3 pt-2 bg-white'>
                            <div class="card" >
                                <img class="card-img-top" src="${porudzbina.slika}" alt="Card image cap">
                                <div class="card-body">
                                    <h6 class="card-title">Naziv: ${porudzbina.naziv}</h6>
                                    <h6 class="card-title">Cena: ${porudzbina.cena}</h6>
                                    <h6 class="card-title">Kategorija: ${kategorija.find(element => element.id === porudzbina.kategorija).naziv}</h6>
                                    <h6 class="card-title">Ukus: ${ukus.find(element => element.id === porudzbina.ukus).naziv}</h6>
                                   <b>Opis:</b>
                                    <p class="card-text">${porudzbina.opis}</p>
                                </div>
                                <div class="card-footer ">
                                    <button class='btn btn-danger form-control' onClick="obrisi(${porudzbina.id})">Obrisi</button>
                                </div>
                            </div>
                        </div>
                    `
    ...
    function obrisi(id) {
        ...
        $.post('server/porudzbina/delete.php', { id }).then(res => {
            ...
            porudzbina = porudzbina.filter(element => element.id != id);
            render();
        })
    }
</script>
